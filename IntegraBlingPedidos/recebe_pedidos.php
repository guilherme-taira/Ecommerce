...
$data2 = preg_replace($patterns, $replace, $dataFinal->format('Y-m-d'));
      ...
      $filter = "dataEmissao[$data2 TO $data1];idSituacao[6,9]";
      $outputType = "json";
          ...
          foreach ($pedido as $produtos) {
            ...
            if (count($count) > 0) {
              ...
              echo "
                  ...
                  <li class='list-group-item'><strong>Número Pedido: {$pedido->pedido->numeroPedidoLoja} - Pedido Já Cadastrado!</strong> <hr> Nome: {$pedido->pedido->cliente->nome}    - CPF: {$pedido->pedido->cliente->cnpj}    - Endereço: {$pedido->pedido->cliente->endereco},{$pedido->pedido->cliente->numero},{$pedido->pedido->cliente->bairro}</li>
                  </ul>
                  </div>  
                  ";
            ...
            <ul class='list-group list-group-flush'>
            <li class='list-group-item'><strong>Número Pedido: {$pedido->pedido->numeroPedidoLoja} - Cadastrado com Sucesso</strong><hr> Nome: {$pedido->pedido->cliente->nome}    - CPF: {$pedido->pedido->cliente->cnpj}    - Endereço: {$pedido->pedido->cliente->endereco},{$pedido->pedido->cliente->numero},{$pedido->pedido->cliente->bairro}</li>
            </ul>
            </div>  
            ";
                try {
