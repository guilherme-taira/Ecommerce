...
class SearchPriceUelloFrete{
    ...
    function __construct($origem,$destino,$peso,$dimensao,$ValorNf)
    {
        $this->origem = $origem;
    ...
    function get($resource){
        ...
        **/
        ...
        $prazoMaxNode->appendChild($prazoMaxNodeValue);    
         ...
         // NÓ IMAGEM FRETE
         $ImagemNodeValue = $dom->createTextNode('https://portaldocomputador.com.br/SistemaPedidos/teste/Uello/logo_uello.png');
        ...
        $dom->save('xmls/'.uniqid()."_".date('Y-m-d').'.xml');

        // VERIFICA SE ESTA DENTRO DO RANGE DAS CIDADE DO CONTRATO
        if(empty($res->data->price) && floatval($this->getPeso()) >= 50){
            return "";
        }else if(floatval($res->data->price) > 0.00 && floatval($this->getPeso()) <= 49.99){
            return $dom->saveXML();
        }else{
            return "";
        }
...
$SearchPriceUelloFrete = new SearchPriceUelloFrete($origem,$destino,$Peso,$Dimensao,$ValorNf);
