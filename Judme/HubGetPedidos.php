...
echo "DATA FINAL  ->>>> $data2";
      ...
      $Orders = new GetPedidos($_SESSION['access_token_tray'],$pagina,$data1,$data2); // 71 ultima atualizacao
      ...
      echo "<div class='container'><div class='row align-items-start'><h3><span class='badge bg-success'>PAGINA : {$pagina} </span> 
    ...
    foreach ($requisicao->Orders as $Order) {
        ...
        foreach ($dados->Order->ProductsSold as $ProdutoPeso) {
            $peso += $ProdutoPeso->ProductsSold->weight;
        }
        ...
        if($dados->Order->shipment == 'EXPRESSO' && $dados->Order->status == 'AGUARDANDO ENVIO'){
          ...
          if (count($count) > 0) {
            ...
            $chaveNf,$NumeroNf,$dataNf,$serieNf,$TotalNf,$peso,$cliente->Customer->cpf,$pdo2,$cliente->Customer->complement);
            // GRAVA NO BANCO
