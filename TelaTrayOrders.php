...
$GetPedidos = new GetPedidos($_SESSION['access_token_tray'], $page,$dataInicial,$dataFinal,'A ENVIAR YAPAY');
...
if (intval($orders->paging->total) <= intval($orders->paging->offset)) {
    ...
    <a href='TelaTrayOrders.php?" . voltaPagina($page) . "'><button class='btn btn-outline-warning' type='button'>Voltar</button></a>
    ...
    echo "<div class='d-grid gap-2 d-md-flex justify-content-md-end'>
    <a href='TelaTrayOrders.php?" . avancaPagina($page) . "'><button class='btn btn-outline-success' type='button'>Próxima</button></a>
    <a href='TelaTrayOrders.php?".limpaFiltros()."'><button class='btn btn-outline-danger' type='button'>Limpar Filtros</button></a>
    </div>";
}else {
    echo "<div class='d-grid gap-2 d-md-flex justify-content-md-end'>
...
foreach ($orders->Orders as $order) {
    ...
    echo "<tbody>
        <td><a href='TelaTrayOrder.php?numpedido={$order->Order->id}'>{$order->Order->id}</a></div></td>
    ...
    echo "<td>{$order->Order->payment_form}</td>
        ...
        <td><a href='TrayCancelPedido.php?order={$order->Order->id}'><span class='badge bg-danger'>Cancelar</span></a></td>
        <td><a href='#'><span class='badge bg-success'>Gerar Nota</span></a></td>
        <td><a href='TelaAtualizaPedido.php?id={$order->Order->id}'><span class='badge bg-warning'>Atualizar Pedido</span></a></td>
      </tr>
    </tbody>";
    //print_r($order->Order);
}
